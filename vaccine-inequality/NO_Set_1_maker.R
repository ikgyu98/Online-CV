...
Oslo_Akershus <- Oslof %>% 
  mutate(location = "Oslo and Akershus")%>%
...
N_Norway <- Nordland_TromsogFinnmark_ %>% 
  mutate(location = "Northern Norway")%>%
  mutate(pop.x = replace_na(pop.x, 241235))%>%
  mutate(pop.y = replace_na(pop.y, 243311))%>%
  mutate(vnum.x = replace_na(vnum.x, 0))%>%
  mutate(vnum.y = replace_na(vnum.y, 0))%>%
  mutate(country.x = replace_na(country.x, "Norway"))%>%
  mutate(resolution.x = replace_na(resolution.x, 1))%>%
...
N_Norway$pop <- as.numeric(N_Norway$pop.x) + as.numeric(N_Norway$pop.y)
N_Norway$vnum <- as.numeric(N_Norway$vnum.x) + as.numeric(N_Norway$vnum.y)
N_Norway <- N_Norway %>%
  relocate(pop, .after = location)%>%
  mutate(pop.x = NULL)%>%
  mutate(pop.y = NULL)%>%
...
SE_Norway <- Viken_Vestfold %>% 
  mutate(location = "South-Eastern Norway")%>%
  mutate(pop.x = replace_na(pop.x, 1241165))%>%
  mutate(pop.y = replace_na(pop.y, 419396))%>%
  mutate(vnum.x = replace_na(vnum.x, 0))%>%
  mutate(vnum.y = replace_na(vnum.y, 0))%>%
  mutate(country.x = replace_na(country.x, "Norway"))%>%
  mutate(resolution.x = replace_na(resolution.x, 1))%>%
...
SE_Norway$pop <- as.numeric(SE_Norway$pop.x) + as.numeric(SE_Norway$pop.y)
SE_Norway$vnum <- as.numeric(SE_Norway$vnum.x) + as.numeric(SE_Norway$vnum.y)
SE_Norway <- SE_Norway %>%
  relocate(pop, .after = location)%>%
  mutate(pop.x = NULL)%>%
  mutate(pop.y = NULL)%>%
...
W_Norway <- Vestland_MøreogRomsdal %>% 
  mutate(location = "Western Norway")%>%
  mutate(pop.x = replace_na(pop.x, 636531))%>%
  mutate(pop.y = replace_na(pop.y, 265238))%>%
  mutate(vnum.x = replace_na(vnum.x, 0))%>%
  mutate(vnum.y = replace_na(vnum.y, 0))%>%
  mutate(country.x = replace_na(country.x, "Norway"))%>%
  mutate(resolution.x = replace_na(resolution.x, 1))%>%
...
W_Norway$pop <- as.numeric(W_Norway$pop.x) + as.numeric(W_Norway$pop.y)
W_Norway$vnum <- as.numeric(W_Norway$vnum.x) + as.numeric(W_Norway$vnum.y)
W_Norway <- W_Norway %>%
  relocate(pop, .after = location)%>%
  mutate(pop.x = NULL)%>%
  mutate(pop.y = NULL)%>%
...
total <- rbind(Agder_Rogaland, Hedmark_Oppland, N_Norway, SE_Norway, W_Norway, Trøndelag, Oslo_Akershus)

total_ <- total%>%
  mutate(pop = NULL)%>%
  mutate(pop =
           case_when(location == "Agder and Rogaland" ~ "787123", 
                     location == "Hedmark and Oppland" ~ "371385",
                     location == "Northern Norway" ~ "484546",
                     location == "Oslo and Akershus" ~ "693494",
                     location == "South-Eastern Norway" ~ "1660561",
                     location == "Trøndelag" ~ "468702",
                     location == "Western Norway" ~ "901769",
                     )
  )%>%
...
write_csv(total_,"Desktop/NO_Set_1.csv")
table(total_$pop)
...
#see <- filter(total, location == "South-Eastern Norway")
...
# "Hedmark and Oppland") ~ "Hedmark,Oppland", >>Innlandet 370603
# "Northern Norway") ~ "Nordland,Troms og Finnmark", 482513
# "Oslo and Akershus") ~ "Oslo (f),Akershus",
# "South-Eastern Norway") ~ "Viken,,Vestfold og Telemark",482513
# "Western Norway") ~ ",Vestland,Møre og Romsdal" 904365
# Trøndelag
